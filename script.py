...
# 2️⃣ CONFIG
# -----------------------------
NOTES_FOLDER = "/Users/yourname/iCloudNotesExport"  # <-- replace with your folder path
LMSTUDIO_API = "http://localhost:11434/v1/chat/completions"
MODEL_NAME = "Phi-3-mini-4k-instruct.Q4_K_M"
MAX_KEYWORDS = 15
BACKLINK_THRESHOLD = 2  # minimum shared keywords to create a backlink
...
def process_notes():
    ...
    for note, data in notes_data.items():
        ...
        post["backlinks"] = backlinks.get(note, [])
...
if __name__ == "__main__":
    process_notes()   words = text.split()
    words = [w for w in words if w not in STOPWORDS and len(w) > 3]
    freq = Counter(words)
    return [word for word, _ in freq.most_common(MAX_KEYWORDS)]
